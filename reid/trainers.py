...
from .loss import TripletLoss, NewTripletLoss
...
class BaseTrainer(object):
    def __init__(self, model, criterion_cro, criterion_tri):
        super(BaseTrainer, self).__init__()
        self.model = model
        self.criterion_cro = criterion_cro
...
class Trainer(BaseTrainer):
    def _parse_data(self, inputs):
        ...
        # inputs, targets = Variable(imgs.cuda()), Variable(pids.cuda())
        return inputs, targets

    def _forward(self, inputs, targets):
        outputs, features = self.model(*inputs)
        # if isinstance(self.criterion_cro, torch.nn.CrossEntropyLoss):
        # loss_c = self.criterion_cro(outputs, targets)

        # elif isinstance(self.criterion_cro, TripletLoss):
        loss_c = self.criterion_cro(outputs, targets)

        # if isinstance(self.criterion_tri, torch.nn.CrossEntropyLoss):
        # loss_t = self.criterion_tri(features, targets)

        # elif isinstance(self.criterion_tri, TripletLoss):
        loss_t = self.criterion_tri(features, targets)
